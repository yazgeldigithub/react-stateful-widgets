...
export default function Programmers() {
	...
	const getNameOfFeatured = () => {
		...
		return programmers.filter(programmer => programmer.id == featuredID)[0].name;
		// let name = programmers.filter(programmer => programmer.id == featuredID);
		// return name[0].name;
	};
	...
	const style = {
		fontSize: '1.5em',
		marginTop: '0.5em',
		color: featuredID === null ? 'royalblue' : 'gold' // 🤔 color turns to gold, when celebrating NOT FINISHED
	};
			...
			<div id="featured" style={style}>
				{
					...
					// false === '' ? `🎉 Let's celebrate ${getNameOfFeatured()}! 🥳` : 'Pick an awesome programmer'
